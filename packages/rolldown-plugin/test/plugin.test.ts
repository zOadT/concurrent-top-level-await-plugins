...
import { CustomPluginOptions, rolldown, RolldownBuild } from "rolldown";
...
describe("rolldown-plugin", () => {
	...
	it("forwards import attributes to resolve", async () => {
		...
		const rolldownCalls: CallRecords = {};
			...
			plugins: [
				{
					...
					resolveId(source, _importer, options) {
						...
						} else if (!(source in rolldownCalls)) {
							rolldownCalls[source] = options;
						} else {
		...
		expect(pluginCalls).toStrictEqual(rolldownCalls);
	});
	...
	describe("dynamic imports", () => {
		...
		it.skip("handles dynamic entry cycles", async () => {
